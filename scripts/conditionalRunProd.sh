...
# get the arguments
##
options=$(getopt -l debug,builder_name:,build_number:,script_dir:,ccd_count:,astro_data: -- "$@")
...
do
        case $1 in
            ...
            --astro_data) ASTRO_DATA=$2; shift 2;;
...
if [ "$SCRIPT_DIR" == "" ] || [ "$BUILDER_NAME" == "" ] || [ "$BUILD_NUMBER" == "" ] || [ "$CCD_COUNT" == "" ] || [ "$ASTRO_DATA" == "" ]; then
    echo "usage: $0 --script_dir <buildbot script dir> --builder_name <name> --build_number <#> --ccd_count <#>  --astro_data <eups version>"
    exit 1
fi

##
# grab the version of datarel from the manifest.list file
##
...
$SCRIPT_DIR/runManifestProduction.sh  --ccdCount $CCD_COUNT --runType buildbot --astro_data $ASTRO_DATA --builder_name "$BUILDER_NAME" --build_number $BUILD_NUMBER  --manifest $LAST_SUCCESSFUL_MANIFEST --beta
