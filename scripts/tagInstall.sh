#! /bin/bash
#
# (based on newinstall.sh)
# Usage: sh tagInstall.sh [package ...]
#
...
if [ -z "$owneups" ]; then
    ...
    cd $LSST_HOME/eups || exit $BUILDBOT_FAILURE
    [ -e default ] && rm default
    ln -s $eupsver default
    cd $LSST_HOME || exit $BUILDBOT_FAILURE
...
echo "------------Completed install of lssteups; setup lssteups"

[ -d "EupsBuildDir" ] && rm -rf EupsBuildDir
setup lssteups

echo "------------Completed setup of lssteups"
eups list
...
echo "--------Completed install of 'lsst'"
eups list
echo "Setup 'lsst'"
...
while [ $# -gt 0 ]; do
    echo "Installing -t $TAG $1"
    eups distrib install -v  -t $TAG $1 || exit $BUILDBOT_FAILURE
    echo "-------Completed install of -t $TAG $1"
    eups list
    shift
done
